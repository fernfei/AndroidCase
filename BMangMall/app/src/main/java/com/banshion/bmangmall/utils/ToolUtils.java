package com.banshion.bmangmall.utils;
...
//      MyToast.showToast("您的系统版本过低，不支持指纹功能");
      ...
      if (!fingerprintManager.isHardwareDetected()) {
        ToastUtil.showMsg(mContext, "您得手机不支持指纹功能", Toast.LENGTH_SHORT);
//        MyToast.showToast("您的手机不支持指纹功能");
        return false;
      } else if (!keyguardManager.isKeyguardSecure()) {
        ToastUtil.showMsg(mContext, "您还未设置锁屏，请先设置锁屏并添加一个指纹", Toast.LENGTH_SHORT);
//        MyToast.showToast("您还未设置锁屏，请先设置锁屏并添加一个指纹");
        return false;
      } else if (!fingerprintManager.hasEnrolledFingerprints()) {
        ToastUtil.showMsg(mContext, "您至少需要在系统设置中添加一个指纹", Toast.LENGTH_SHORT);
//        MyToast.showToast("您至少需要在系统设置中添加一个指纹");
  ...
  /**
   * @TargetApi(23)使高版本API的代码在低版本SDK不报错
   */
  @TargetApi(23)
